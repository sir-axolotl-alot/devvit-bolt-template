...
import devvitMockedResponses, {
  DevvitMessageHandler,
} from './DevvitMockedResponses';
...
class DevvitBackendHandler implements DevvitMessageHandler {
  ...
  setMessageHandler(handler: (message: DevvitSystemMessage) => void) {
    window.addEventListener('message', (ev: MessageEvent) =>
      handler(ev.data as DevvitSystemMessage)
    );
  }
...
export class DevvitClient {
  ...
  public initialize(useMockedResponses: boolean = false): void {
    ...
    if (useMockedResponses) {
      ...
      console.log(
        'DevvitClient',
        'Connected to Devvit Backend. Use `devvit playtest` to test your app'
      );
    }
  ...
  public on(
    ...
    handler: (data: DevvitMessage) => void
  ): void {
