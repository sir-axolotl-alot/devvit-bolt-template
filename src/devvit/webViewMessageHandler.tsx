...
async function fetchPostData(
  ...
  webView: UseWebViewResult<DevvitMessage>,
...
async function setUserScore(
  ...
  if (message.type === 'setUserScore') {
    console.log(
      'Devvit',
      'Received user score from web view:',
      message.data.score
    );
    const redisService = createRedisService(context);
    const username = (await context.reddit.getCurrentUsername()) ?? 'anon';
    await redisService.setLeaderboardEntry(username, message.data.score);
    console.log(
      'Devvit',
      'Saved user score to leaderboard:',
      username,
      message.data.score
    );
    webView.postMessage({
...
async function fetchUserData(
  ...
  if (message.type === 'fetchUserData') {
    ...
    webView.postMessage({
      ...
      data: {
        redditUser: {
          userId: redditUserData!.id,
          username: redditUserData!.username,
        },
        dbUser: dbUserData,
        error: '',
      },
...
async function setUserData(
  ...
  if (message.type === 'setUserData') {
    console.log(
      'Devvit',
      'Received user data from web view:',
      message.data.userData
    );
    const redisService = createRedisService(context);
    await redisService.saveUserData(message.data.userId, message.data.userData);
    webView.postMessage({
...
export async function buyProductResponse(
  result: OnPurchaseResult,
  webView: UseWebViewResult<DevvitMessage>,
  context: Devvit.Context
) {
...
async function fetchOrders(
  ...
  const webviewOrders = orders.map((order) => {
    if (!order.products[0]) {
      throw new Error(`Order ${order.id} has no products`);
    }
...
export async function handleWebViewMessages(
  message: WebViewMessage,
  webView: UseWebViewResult<DevvitMessage>,
  context: Devvit.Context,
  ...
  switch (message.type) {
    case 'webViewReady':
      await webView.postMessage({
        type: 'initialData',
        data: { userId: context.userId ?? 'anon', postId: context.postId! },
      });
      break;
    case 'fetchPostData':
